...
class cls_s3(object):
    ...
    def getObject(self,symbol,bucket,key):
        actual_key = f'{key}/{self.dt}/{symbol}.csv'#'{0}/{1}.csv'.format(key,symbol)
    ...
    def saveObject(self,df,bucket,key):
        ...
        actual_key = f'{key}/{self.dt}/{symbol}.csv'
        csv_buffer = io.StringIO()
        df.to_csv(csv_buffer,index=False)
        response = self.s3.put_object(Bucket = bucket,Key=actual_key,Body=csv_buffer.getvalue())
